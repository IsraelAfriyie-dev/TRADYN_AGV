...
# Integration example for TRADYN system
def integrate_with_tradyn_navigation(planning_module, prediction_module, 
                                   failure_system: FailureLearningSystem):
    """
    Example integration with TRADYN navigation system.
    This shows how to integrate failure recognition with existing planning and prediction.
    """
    
    def enhanced_navigate_terrain(start_pos, goal_pos, terrain_data, robot_state):
        """Enhanced navigation function with failure recognition."""
        
        # Create contexts
        terrain_context = create_terrain_context_from_data(terrain_data)
        robot_context = create_robot_context_from_state(robot_state)
        ...
        similar_experiences, lessons = failure_system.start_navigation(
            navigation_id, start_pos, goal_pos, terrain_context, robot_context
        )
        
        # Get terrain insights to inform planning
        insights = failure_system.get_terrain_insights(terrain_context)
        print(f"Terrain insights: {insights['common_failures'][:3]}")  # Top 3 common failures
        ...
        while attempt < max_attempts:
            ...
            try:
                # Plan trajectory (with failure insights)
                planned_trajectory = planning_module.plan_path(
                    start_pos, goal_pos, terrain_context, robot_context,
                    risk_factors=insights.get('risk_factors', []),
                    recommended_approaches=insights.get('recommended_approaches', [])
                )
                
                # Execute navigation with monitoring
                actual_trajectory = []
                success = False
                
                for i, waypoint in enumerate(planned_trajectory):
                    # Simulate navigation step
                    current_time = i * 0.1  # 0.1 second steps
                    current_pos = waypoint
                    energy_used = i * 0.01  # Simulate energy consumption
                    battery_level = 1.0 - energy_used
                    
                    # Update failure monitoring
                    failure_system.update_navigation_state(
                        current_time, current_pos, energy_used, 
                        velocity=np.array([0.1, 0.1]), battery_level=battery_level
                    )
                    actual_trajectory.append(current_pos)
                    
                    # Check for failures
                    failures = failure_system.check_for_failures(
                        goal_pos, battery_level, terrain_context,
                        planned_trajectory, actual_trajectory
                    )
                    
                    if failures:
                        # Failure detected!
                        failure_info = failures[0]  # Handle first failure
                        print(f"Failure detected: {failure_info['failure_type']}")
                        
                        # Record failure
                        failure_event = failure_system.record_failure(
                            failure_info, terrain_context, robot_context,
                            start_pos, goal_pos, current_pos,
                            planned_trajectory, actual_trajectory,
                            energy_used, current_time
                        )
                        ...
                        print(f"Suggested recovery strategies: {strategies}")
                        
                        # Try recovery (simplified example)
                        if strategies:
                            recovery_strategy = strategies[0]
                            print(f"Attempting recovery: {recovery_strategy}")
                            
                            # Simulate recovery attempt
                            recovery_success = attempt < max_attempts  # Simple success model
                            
                            lessons_learned = []
                            if recovery_success:
                                lessons_learned.append(f"Recovery strategy '{recovery_strategy}' worked on {terrain_context.terrain_type}")
                            else:
                                lessons_learned.append(f"Recovery strategy '{recovery_strategy}' failed on steep {terrain_context.terrain_type}")
                                
                            failure_system.record_recovery_attempt(
                                recovery_strategy, recovery_success, lessons_learned
                            )
                ...
                if success:
                    ...
                    failure_system.finish_navigation(success=True)
                    return actual_trajectory, True
                    
            except Exception as e:
                print(f"Navigation attempt {attempt} failed with exception: {e}")
                
        # All attempts failed
        print("All navigation attempts failed")
        failure_system.finish_navigation(success=False)
        return actual_trajectory, False
    
    return enhanced_navigate_terrain
...
if __name__ == "__main__":
    ...
    print(f"Steep grass vs Similar grass similarity: {steep_grass_slope.similarity(similar_grass):.3f}")
    
    print("\nSystem ready for integration with TRADYN navigation!")
    print("Use the FailureLearningSystem class to add failure recognition to your navigation pipeline.")failure_event = None
