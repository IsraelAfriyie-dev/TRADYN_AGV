...
def get_config_updates(updates):
    ...
    for upd in updates:
        if upd == "":
            continue
        if upd[-5:] == ".json":
            # This is a config file specification
            named_configs.append(upd)
            continue
...
class RestartableExperiment(Experiment):
    ...
    def _build_restart_from_args(self, args, parent_run_id):
        ...
        restart_args = ["dummy"]
        if args.comment:
            restart_args += f'--comment="{args.comment}"'
        restart_args.extend(
            ["with", f"{config_location}", f"parent_id={parent_run_id}"]
        )
        return restart_args
    ...
    def _build_restart_base_args(self, args, parent_run_id):
        ...
        if args.comment:
            restart_args += f'--comment="{args.comment}"'
        restart_args.extend(["with", f"{config_location}"])
        return restart_args
